...
Procedure Tfmscueind.FormActivate(Sender: TObject);
Begin
   If DM1.xSgr = 'R' Then
   Begin
...
Procedure Tfmscueind.btnSalirClick(Sender: TObject);
Begin
   Close;
End;

Procedure Tfmscueind.pplblNombreMesPrint(Sender: TObject);
Begin
   pplnommes.Caption := DM1.NombreMes(DM1.cdsQry22.FieldByName('MES').AsString);
End;

Procedure Tfmscueind.pplbluser_aPrint(Sender: TObject);
Begin
   If DM1.cdsQry19.RecordCount > 0 Then pplbluser_a.Visible := False;
End;

Procedure Tfmscueind.ppLabel95Print(Sender: TObject);
Begin
...
Procedure Tfmscueind.ppLabel180Print(Sender: TObject);
Begin
   ppLabel191.Visible := True;
End;

Procedure Tfmscueind.ppLblUserPrint(Sender: TObject);
Begin
...
Procedure Tfmscueind.btnprocesarClick(Sender: TObject);
Var
...
Begin
   ...
   Begin
      ...
      dbgcueind.Top := 61;
      dbgcueind.Left := 8;
      xSql := 'DELETE T_CTAIND1 WHERE ASOID = ' + QuotedStr(DM1.cdsAsociado.FieldByName('ASOID').AsString);
      ...
      xSql := 'SELECT LUG_PRO,COD_USE,COD_MOD,NRO_VEZ,LIB_ELE,ACUMESANT,ACUMULADO,CAPITAL, '
            + 'INTCAPACU,BONOS,FEC_ING,FEC_CES,ANOSERF,ANOSER,FEC_PROC,FLG_PROC, '
            + 'TIP_CLI,ANO,MES,ASOID,TRI,COD_PAG,TRANSMTODEV,TRANSMTO '
...
Procedure Tfmscueind.rgtiprepClick(Sender: TObject);
Begin
   pnlCCI.visible:=False;
...
Procedure Tfmscueind.btnimprimeClick(Sender: TObject);
Var
...
Begin
   ...
   +' GROUP BY ASOID, TRANSANO, TRANSMES ORDER BY ASOID, TRANSANO, TRANSMES) B, APO201 C WHERE A.APOANO = B.TRANSANO(+) AND A.APOMES = B.TRANSMES(+)'
   ...
   ppltotsalpenpagest.Caption := 'TOTAL APORTES PENDIENTE DE PAGO S/. '+FloatToStrF(xmonpen,ffNumber,9,2);
...
Procedure Tfmscueind.ppMemo3Print(Sender: TObject);
Begin
   If DM1.cdsQry19.RecordCount > 0 Then
...
Procedure Tfmscueind.ppMemo4Print(Sender: TObject);
Begin
   If DM1.xSgr = 'E' Then ppMemo4.Visible := False;

End;

Procedure Tfmscueind.ppLabel20Print(Sender: TObject);
Begin
   If DM1.xSgr = 'E' Then
...
Procedure Tfmscueind.ppLabel132Print(Sender: TObject);
Begin
   If DM1.xSgr = 'E' Then
...
Procedure Tfmscueind.pplanomesPrint(Sender: TObject);
Var
   xMes: String;
Begin
...
procedure Tfmscueind.FormClose(Sender: TObject; var Action: TCloseAction);
begin
